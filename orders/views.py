...
def sslc_complete(request,val_id,tran_id):
    ...
    # order_rs = Order.objects.get(user=request.user, is_ordered=False ) 
    # order = order_rs[0]
    order = Order.objects.filter(user=request.user , is_ordered = False ).order_by('-created_at')[0]
    # order = Order.objects.filter(user=request.user, is_ordered=False )[-1]
    # payment_data = request.POST
    # status = payment_data['status']
    ...
    try:
        ...
        context = {
            'order':order,
            'order_product':ordered_products,
    ...
    # try:
    #     print("enter try blog ")
    #     order = Order.objects.get(order_number = val_id , is_ordered = True)
    #     order_product = OrderProduct.objects.filter(order_id = order.id)
    #     subtotal = 0
